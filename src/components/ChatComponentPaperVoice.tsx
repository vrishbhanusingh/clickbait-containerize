...
const ChatComponentPaperVoice = ({ paperId }: Props) => {
  ...
  const stopRecording = async () => {
    if (mediaRecorderRef.current) {
      ...
      mediaRecorderRef.current.addEventListener("stop", async () => {
        ...
        speechToText(audioBlob, async (data) => {
          voicePromptRef.current = true;
          const transcription = await transcribeAudio(data);
          if (transcription.status) {
            append({
  ...
  return (
    <div className="flex flex-col h-full bg-white shadow-lg rounded-lg overflow-hidden">
      <div className="bg-customTeal text-white p-3">
        <h3 className="text-lg font-bold">AI Voice Assistant</h3>
      </div>
      <div className="flex-1 p-2 overflow-y-auto scrollbar-hide" id="message-container">
        <MessageList messages={messages} isLoading={isLoading} />
      </div>
      <div className="p-2 flex justify-center items-center">
        {isLoading || audioLoading ? (
          ...
          <Button
            ...
            onTouchStart={handleRecording}
            onTouchEnd={handleRecording}
            className="h-fit rounded-full bg-red-900 p-4 shadow-lg hover:bg-red-600"
          >
      ...
      <div className="p-2 bg-gray-50 border-t">
        <form onSubmit={handleSubmit} className="flex">
          <Input
            ...
            disabled={isLoading}
          />
          <Button type="submit" className="ml-2 bg-customTeal" disabled={isLoading}>
            <Send className="h-4 w-4" />
          </Button>
        </form>
      </div>
