...
import { customProvider } from '~/app/actions/custom-provider';
import { getContextPdf, getContextWeb } from '~/lib/context';
import { db } from '~/server/db';
import { papers, messages as _messages } from '~/server/db/schema';
import { NextResponse } from 'next/server';
import { eq } from 'drizzle-orm';
...
export async function POST(req: Request) {
  try {
    ...
    const systemPrompt = `AI assistant is a brand new, powerful, human-like artificial intelligence.
      ...
      AI assistant will take into account PDF CONTEXT BLOCK, WEB CONTEXT BLOCK that is provided in a conversation.
      If the WEB CONTEXT BLOCK is empty, AI assistant will say, "I am not able to retrieve information for this question."
      AI assistant if asked a question requiring real time information that needs access to the internet, it will use real time information from the WEB CONTEXT BLOCK.
      If the context does not provide the answer to question, the AI assistant will say, "I'm sorry, but I don't know the answer to that question".
      AI assistant will not apologize for previous responses, but instead will indicate new information was gained.
      AI assistant will not invent anything that is not drawn directly from the context.
      AI assistant will not make the response too long or too short. The maximum words in the response is 150 words.`;
    ...
    // console.log(systemPrompt);
    const model = customProvider('llama3-fintune');
    // const result = await model.doGenerate(systemPrompt, messages);
    const result = await model.doGenerate(systemPrompt);
    await db.insert(_messages).values({
      paperId: paperId,
      content: result.text,
      role: 'system',
