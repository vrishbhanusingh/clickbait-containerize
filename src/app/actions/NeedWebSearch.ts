  ...
  let WebSearchNeeded = false;
    try {
        const { object: result } = await generateObject({
          ...
          system: 'You are tasked to determine whether the message in the MESSAGE BLOCK needs web search results. You are given the PDF CONTEXT BLOCK which has relavant information from the PDF of a research paper. Please determine whether the message can be answered with just the information in the PDF CONTEXT BLOCK or if it needs a web search for more information. The answer should be yes if it needs web search results and no if it does not need a web search. For questions related to the paper in the PDF it is beneficial if you get more information from the web.',
