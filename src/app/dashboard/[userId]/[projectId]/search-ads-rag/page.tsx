...
const Page = ({ params: { userId } }: Props) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4 ml-16 sm:ml-[200px]">
      <div className="w-full max-w-2xl mb-8">
        <h1 className="text-2xl font-bold text-center mb-6">Semantic Search on the NASA/ADS Database</h1>
        <div className="bg-white shadow-lg rounded-lg p-6">
          ...
          <Button
            ...
            className="bg-customTeal text-white px-4 py-2 rounded hover:bg-teal-800 w-full"
          >
            Search
          </Button>
        </div>
      ...
      <div className="w-full max-w-2xl">
        ...
        {data && (
          <div className="grid grid-cols-1 gap-4">
            {data.map((item, index) => (
              <div key={index} className="border border-gray-300 shadow-lg rounded-lg p-8 bg-white">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center">
                    <h3 className="text-xl font-bold mr-2">{item.metadata.title}</h3>
                    {item.metadata.citation_count > 50 && (
                      <span className="bg-red-500 text-white px-2 py-1 text-xs rounded mr-2">Highly cited</span>
                    )}
                    {item.metadata.read_count > 30 && (
                      <span className="bg-blue-500 text-white px-2 py-1 text-xs rounded">Highly read</span>
                    )}
                  </div>
                  <button
                    onClick={() => window.open(item.metadata.PDF_link_url, '_blank')}
                    className="bg-customTeal text-white px-3 py-1 text-xs rounded hover:bg-teal-800"
                  >
                    Open Paper
                  </button>
                </div>
                <p className="text-gray-700 mb-2">
                  {expanded[item.id]
                    ...
                    : `${item.metadata.abstract.substring(0, 200)}...`}
                  <button
                    onClick={() => toggleExpand(item.id)}
                    className="text-teal-600 hover:text-teal-800 ml-2"
                  >
